...
class DriftGuardSourceAdmissionTests(unittest.TestCase):
    ...
    def test_unknown_top_level_authority_claim_is_rejected(self) -> None:
        errors = _mutated(lambda spec: spec.__setitem__("runtime_integration_authorized", True))
        self.assertTrue(any("top-level machine contract keys" in error for error in errors))

    def test_snapshot_semantics_cannot_be_removed(self) -> None:
        errors = _mutated(lambda spec: spec["snapshot_semantics"].remove("SNAPSHOT_REVIEW_STATE_NE_LIVE_REVIEW_STATE"))
        self.assertTrue(any("snapshot semantics" in error for error in errors))

    def test_live_currentness_claim_cannot_be_added(self) -> None:
        errors = _mutated(lambda spec: spec["claim_ceiling"].remove("NO_LIVE_SOURCE_CURRENTNESS_CLAIM"))
