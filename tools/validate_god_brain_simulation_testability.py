...
def validate_simulation_testability(root: Path) -> list[str]:
    ...
    for marker in ("NO_SIMULATION_FACT_CLAIM","NO_NONSIMULATION_FACT_CLAIM","NO_SIMULATION_PROBABILITY_ASSIGNMENT"):
        if marker not in ceilings:
            errors.append(f"claim ceiling missing {marker}")
